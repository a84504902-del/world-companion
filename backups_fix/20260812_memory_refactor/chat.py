...
async def chat_handler(request):
    ...
    # ===== 组装每轮注入的上下文（人物卡 → 关系 → 世界状态 → 摘要 → 检索记忆）=====
    context_parts = []

    # 1. 人物卡（含画像 description，放最前）
    people = db.get_people(_current_session_id)
    if people:
        people_lines = []
        for p in people:
            desc = (p.get("description") or "").strip()
            age = f"（{p['age']}岁）" if p.get("age") else ""
            people_lines.append(f"- {p['name']}{age}" + (f"：{desc}" if desc else ""))
    ...
    if relations:
        ...
        for r in relations:
            ...
            rel_lines.append(f"- {name_a} 与 {name_b} 是{r['relation_type']}关系")
    ...
    # 后台索引对话原文块（替代旧的自动事实提取：
    # 提取有损+幻觉，产生 100+ 脏记忆；原文切块无损、按话题检索即可）
    try:
        asyncio.get_event_loop().run_in_executor(
            None, lambda: memory_retriever.index_conversation_chunks(_current_session_id)
        )
    except Exception as e:
        logger.error("原文块索引失败: %s", e)
...
async def _auto_summarize():
    ...
    try:
        ...
        # 1) 生成摘要并落库（关键信息：人物关系、重要约定必须保留）
        messages = [
            {"role": "system", "content": "你是摘要助手。请用简洁中文总结以下对话的核心情节与关键信息，100字以内。人物关系的变化和重要约定必须保留。"},
